...
describe('components/form/sw-url-field', () => {
    ...
    it('properly detects SSL', async () => {
        const SSL_URL = 'https://heyframe.net';
        const NON_SSL_URL = 'http://heyframe.net';
        const URL_WITHOUT_PROTOCOL = 'heyframe.net';
    ...
    it('removes any protocol', async () => {
        const HTTP_URL = 'http://heyframe.net';
        const HTTPS_URL = 'https://heyframe.net';
        const FILE_URL = 'file://heyframe.net';
        const EXPECTED_URL = 'heyframe.net';
    ...
    it('allows empty values', async () => {
        const INITIAL_URL = 'https://heyframe.net';
        const URL_WITHOUT_PROTOCOL = 'heyframe.net';
        const EXPECTED_URL = '';
    ...
    it('should update empty values', async () => {
        const wrapper = await createWrapper({
            props: {
                value: 'https://heyframe.net',
            },
        });
        await flushPromises();

        const input = wrapper.find('input');
        expect(input.element.value).toBe('heyframe.net');
    ...
    it('adds the trailing slash if the prop is set', async () => {
        ...
        await input.setValue('heyframe.net');
        await input.trigger('blur');
        await nextTick();
        expect(wrapper.vm.currentUrlValue).toBe('heyframe.net/');
        expect(wrapper.emitted('update:value')).toStrictEqual([
            ['https://heyframe.net/'],
        ]);
    });
