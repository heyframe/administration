const { Application, Service, Store } = HeyFrame;
...
export default function addCustomerGroupRegistrationListener(loginService) {
    ...
    async function checkForUpdates() {
        if (!HeyFrame.Service('acl').can('customer.viewer')) {
            return;
        }
        ...
        const customers = await customerRepository.search(criteria, HeyFrame.Context.api);
