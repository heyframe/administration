...
describe('src/core/service/usage-data-consent-listener.service.ts', () => {
    ...
    beforeEach(() => {
        ...
        loginService = createLoginService({}, HeyFrame.Context.api);
        serviceContainer = {
            usageDataService: {
                getConsent: () => {
        ...
        HeyFrame.Store.get('usageData').updateConsent({
    ...
    it('should update consent on login', async () => {
        ...
        expect(HeyFrame.Store.get('usageData').isConsentGiven).toBe(true);
        expect(HeyFrame.Store.get('usageData').isBannerHidden).toBe(false);
    });
    ...
    it('should reset the consent if the request fails', async () => {
        ...
        expect(HeyFrame.Store.get('usageData').isConsentGiven).toBe(false);
        expect(HeyFrame.Store.get('usageData').isBannerHidden).toBe(true);
    });
