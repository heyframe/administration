...
export default class FeatureGridTranslationService {
    ...
    _fetchEntities(features, type, filterBy, repo) {
        ...
        return repo.search(criteria, HeyFrame.Context.api).then((items) => {
    ...
    getNameTranslation(item) {
        ...
        if (item.type === 'customField') {
            const language = HeyFrame.Store.get('session').currentLocale;
            const fallback = HeyFrame.Context.app.fallbackLocale;
