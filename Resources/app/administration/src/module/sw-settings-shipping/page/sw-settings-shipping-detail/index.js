...
export default {
    ...
    computed: {
        shippingMethod() {
            return HeyFrame.Store.get('swShippingDetail').shippingMethod;
        },

        currencies() {
            return HeyFrame.Store.get('swShippingDetail').currencies;
        },

        restrictedRuleIds() {
            return HeyFrame.Store.get('swShippingDetail').restrictedRuleIds;
        },
    ...
    methods: {
        createdComponent() {
            if (!this.shippingMethodId) {
                ...
                HeyFrame.Store.get('swShippingDetail').shippingMethod = shippingMethod;
            } else {
                this.loadEntityData();
            }
            this.loadCurrencies();
        ...
        loadCurrencies() {
            ...
            this.currencyRepository.search(criteria, Context.api).then((currencyResponse) => {
                HeyFrame.Store.get('swShippingDetail').currencies = this.sortCurrencies(currencyResponse);
        ...
        loadEntityData() {
            ...
            this.shippingMethodRepository
                .get(this.shippingMethodId, HeyFrame.Context.api, this.shippingMethodCriteria)
                .then((res) => {
                    HeyFrame.Store.get('swShippingDetail').shippingMethod = res;

                    this.ruleConditionDataProviderService.getRestrictedRules('shippingMethodPrices').then((result) => {
                        HeyFrame.Store.get('swShippingDetail').restrictedRuleIds = this.restrictedRuleIds.concat(result);
                    });
